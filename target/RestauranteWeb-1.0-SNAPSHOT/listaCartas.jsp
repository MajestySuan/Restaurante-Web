<%-- 
    Document   : listaCartas
    Created on : 5/07/2025, 8:29:31 p. m.
    Author     : zudex
--%>
...
<div class="container mt-4">
  <h2>Cartas</h2>
  <a href="CartaController?action=nuevo" class="btn btn-success mb-3">+ Nueva Carta</a>
  <table class="table table-striped">
    <thead><tr>
      <th>Nombre</th><th>Inicio</th><th>Fin</th><th>Activa</th><th>Acciones</th>
    </tr></thead>
    <tbody>
      <c:forEach var="c" items="${listaCartas}">
        <tr>
          <td>${c.nombreCarta}</td>
          <td>${c.fechaInicioVigencia}</td>
          <td>${c.fechaFinVigencia}</td>
          <td>${c.activa?'Sí':'No'}</td>
          <td>
            <a href="CartaController?action=editar&id=${c.idCarta}" class="btn btn-sm btn-primary">Editar</a>
            <a href="CartaController?action=eliminar&id=${c.idCarta}" class="btn btn-sm btn-danger"
               onclick="return confirm('Eliminar esta carta?');">Eliminar</a>
          </td>
        </tr>
      </c:forEach>
    </tbody>
